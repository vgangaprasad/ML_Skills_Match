...
class nonMLDetector(object):
    ...
    def fit(self, X, Y):
        ...
        n = len(X)
        self.num_messages['nonML'] = sum(1 for label in Y if label == 'negative')
        self.num_messages['ML'] = sum(1 for label in Y if label == 'positive')
...
# Iterate over all the items in dictionary and filter items which has even keys
   ...
   # Check if key is even then add pair to new dictionary
            if value >= 5.0:
                newList.append(key) 
        additional_word_list=['scala','postgres','css','javascript','js','net','unix','architechture','sql','linux','motivated','transform']
        # newList.append (additional_word_list)   
